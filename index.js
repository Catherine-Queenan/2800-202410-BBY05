...
app.get('/clientList', businessAuthorization, async (req, res) => {
	...
	// !Currently, the companyName is set to null because there is no system for business view user pages at the time of writing.! (I'm not sure if this is still the case, but I'll leave this comment here to prevent confusion -Filip)
	const userdb = appdb.db(req.session.userdb);
	const clientList = await userdb.collection('clients').find().project({email: 1}).toArray();
	let clientListArray = [];
